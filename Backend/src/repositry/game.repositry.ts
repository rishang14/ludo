...
export class GameRepo {
  ...
  public static async getGame(gameId:string):Promise<Game>{
     const game=await prisma.game.findFirstOrThrow({where:{
        id:gameId
     }}) 
     if(!gameId){
  ...
  public static async removeUserFromTheGame(gameId:string){
    ...
    for(const id of playerIds){
      const updatedUser= await UserRepo.updateUserData(id,{onGoingGame:null});  
      if(!updatedUser){
        throw new Error("Something Went Wrong while removing the user From the game");
      }
    } 
    return true; 
  }
